...
SELECT terminal_id,gateway,gateway_acquirer
  ...
  WHERE gateway in ('enach_npci_netbanking','enach_rbl','nach_citi','nach_icici') and gateway_acquirer = 'citi'
  GROUP BY 1,2,3
)
...
,base as (SELECT t.id as token_id
  ...
  ,md.contact_email
  ,t.frequency
  ,CASE WHEN dd.max_debit_date IS NOT NULL THEN DATE_DIFF(month, cast(dd.max_debit_date as date), CAST('2024-03-31' AS date))
  WHEN dd.max_debit_date IS NULL THEN DATE_DIFF(month, cast(t.created_date as date), CAST('2024-03-31' AS date))
...
pd_df_2 = pd_df_tokens[pd_df_tokens['Dormancy_Bucket'] != '6 Months - 1 Year']
...
print(len(pd_df_2[pd_df_2['gateway_acquirer'] == 'citi']))
print(len(pd_df_2))

# COMMAND ----------

aggregated = pd_df_tokens.groupby(['Dormancy_Bucket', 'Penalty_Applicable', 'gateway_acquirer']).agg({
    'UMRN_NO': 'nunique',   # Distinct count of UMRN_NO
    'merchant_id': 'nunique'  # Distinct count of merchant_id
})
...
df.to_csv('/dbfs/FileStore/tables/Anurag/mandates_shared_by_citi_mapped.csv')
